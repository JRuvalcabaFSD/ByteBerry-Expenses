...
# ByteBerry Expenses Service - Multi-arch Docker Build Script
# Supports: linux/amd64, linux/arm64 (Raspberry Pi 5)

set -e

# Configuration
DOCKER_HUB_REPO="jruvalcabafsd/byteberry-expenses"
IMAGE_NAME="jruvalcabafsd/byteberry-expenses"
...
build_dev() {
    ...
    print_info "Run with: docker run --rm -p 4001:4001 ${IMAGE_NAME}:dev"
}
...
test_image() {
    ...
    CONTAINER_ID=$(docker run -d -p 4001:4001 ${image_tag})
...
usage() {
    ...
    echo "  test [tag]        Test built image (default: byteberry-expenses:dev)"
