...
import { getErrMessage, Injectable, LogContextClass, LogContextMethod, withLoggerContext } from '@shared';
...
@LogContextClass()
@Injectable({ name: 'HealthService', depends: ['Config', 'Uuid', 'Clock', 'Logger', 'HealthRegistry', 'DatabaseHealthChecker'] })
export class HealthService implements Interfaces.IHealthService {
	...
	public async checkHealth(
		...
		const overallStatus = this.determineOverallStatus(dependencies, databaseHealth);
	...
	private async checkDependencies(services: string[]): Promise<Record<string, Interfaces.DependencyResponse>> {
		...
		const checkers = this.healthRegistry.getCheckers();

		await Promise.all(
			Array.from(checkers.entries()).map(async ([name, checker]) => {
				const serviceStartTime = this.clock.timestamp();
				...
				try {
					...
					dependencies[name] = {
						...
						responseTime: this.clock.timestamp() - serviceStartTime,
					};
				} catch (error) {
					dependencies[name] = {
						status: 'unhealthy',
