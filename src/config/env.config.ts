...
export class Config implements IConfig {
	...
	constructor() {
		try {
			...
			this.port = env.get('PORT').default('4001').asPortNumber();
			this.version = pkg.version ?? '0.0.0';
			this.serviceName = env.get('SERVICE_NAME').default('ByteBerry-Expenses').asString();
			...
			this.corsOrigins = this.normalizeUrls(
				env.get('CORS_ORIGINS').default('http://localhost:5173,http://localhost:4003,http://localhost:4001').asArray(',')
			);
		} catch (error) {
