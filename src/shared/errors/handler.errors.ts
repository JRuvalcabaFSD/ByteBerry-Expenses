 ...
 * - Logging the formatted error to the console with the ByteBerry-Expenses namespace
 *
 * @example
 * ```typescript
 * try {
...
const defaultHandler = (error: any) => {
	...
	console.log(`${timestamp} [ByteBerry-Expenses] ${type}: ${message}`);
};
...
function getMessage(error: AppError, type: string): string {
	...
	let logMessage = `${timestamp} [ByteBerry-Expenses] ${Colors.Red}${Colors.Bold}${type ?? 'Error'}:${Colors.Reset} ${error.message}`;
